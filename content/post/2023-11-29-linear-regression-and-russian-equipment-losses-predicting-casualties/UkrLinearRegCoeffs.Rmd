...
Each **coefficient** of a regression model indicates the influence of one additional unit loss of that equipment on the casualty figure.  So, for example, under the fitted model, the average difference in casualties, comparing two days of the same equipment losses but one tank more is `r as.numeric(summaryBroom[5, 2])` more liquidated personnel than the model's average, `r round(coef(RegModAllVars)["(Intercept)"], digits = 0)` Soldiers.  Likewise, for every lost Russian artillery piece, subtract `r as.numeric(summaryBroom[2, 2])` lost personnel.^[The "e" at the end of a regression formula stands for the errors (aka "residuals"), which are the differences between the the model's predicted number of soldiers killed each day using the data on equipment lost and the actual number of soldiers killed that day.]
...
Here is the formula for our model. Soldier deaths, where the first number is the predicted average, can be predicted from this equation, `r paste(round(coeffs[1],1), paste(round(coeffs[-1],1), names(coeffs[-1]), sep=" times ", collapse=" + "), sep="+")` plus an error term, when you plug in the figures for the respective equipment losses.
